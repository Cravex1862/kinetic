...
export const BrandStylingPanel: React.FC<StylingProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"styling" | "backgrounds">(
    "styling",
  );
  ...
  useEffect(() => {
    const activeFamilies = Object.values(fonts)
      .map((f) => f?.fontFamily)
      .filter(Boolean);
      ...
      {activeTab === "styling" ? (
        <>
          ...
          <div className="pt-1 border-t border-gray-900">
            <span className="text-[10px] font-semibold text-gray-500 block mb-2">
              Palette Colors
            </span>
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(swatches).map(([label, color]) => (
                <div
                  key={label}
                  className="flex items-center gap-2 bg-gray-950/40 border border-gray-900/50 rounded-lg px-2 py-1.5 hover:border-gray-800 transition-colors"
                >
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[8px] font-semibold truncate capitalize">
                      {label}
                    </span>
                    <span className="text-[8px] text-gray-400 font-mono leading-none truncate">
                      {color}
                    </span>
                  </div>
                </div>
        ...
        <div className="pt-1">
          <BackgroundSelectorPanel
            ...
            onSelectBackground={(bg) =>
              onSelectBackground && onSelectBackground(bg)
            }
          />
        </div>
      )}
