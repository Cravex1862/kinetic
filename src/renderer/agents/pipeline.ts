...
function buildDesignTokenVars(tokens: DesignTokens): string {
  const esc = (v: string) => v.replace(/'/g, "\\'");
  return [
    `const primaryColor = '${esc(tokens.primaryColor)}';`,
    `const secondaryColor = '${esc(tokens.secondaryColor || tokens.primaryColor)}';`,
    `const accentColor = '${esc(tokens.accentColor)}';`,
    `const semanticColor = '${esc(tokens.semanticColor || '#3b82f6')}';`,
    `const errorColor = '${esc(tokens.errorColor || '#ef4444')}';`,
    `const successColor = '${esc(tokens.successColor || '#22c55e')}';`,
    `const neutralColor = '${esc(tokens.neutralColor || '#64748b')}';`,
    `const surfaceColor = '${esc(tokens.surfaceColor)}';`,
    `const backgroundColor = '${esc(tokens.backgroundColor)}';`,
    `const textColor = '${esc(tokens.textColor)}';`,
    `const fontFamily = '${esc(tokens.fontFamily)}';`,
  ].join("\n");
}
...
async function assembleVideoComposition(
  ...
  const tokenVars = buildDesignTokenVars(designTokens);
