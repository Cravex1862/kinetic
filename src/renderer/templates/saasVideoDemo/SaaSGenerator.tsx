...
const SaaSGenerator: React.FC<AnimationGeneratorProps> = ({
  ...
  const [fonts, setFonts] = useState<Record<FontRow, FontSettings>>(
    (project?.fonts as Record<FontRow, FontSettings>) || defaultFonts,
  );
  ...
  const [scannedExports, setScannedExports] = useState<ScrapedFindings | null>(
    null,
  );
  ...
  const buildRepoContext = (
    findings: ScrapedFindings,
    pack: RepoPackResult | null,
  ): string => {
  ...
  const completeRepoScan = async (results: ScrapedFindings, source: string) => {
    ...
    await customAlert(
      "Scan Complete",
      `Found ${results.components.length} components and ${results.routes.length} routes.${packNote} Generation will use this context.`,
    );
  };
  ...
  const handleSelectFolder = async () => {
    if (!window.electronAPI?.selectDirectory) {
      await customAlert(
        "Feature Unavailble",
        "Selecting Directories is only supported inside the desktop app",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        await customAlert(
          "Scan Complete",
          "Scan completed but no components or colors were detected. Try Skip to continue without repo context.",
        );
        return;
      }
  ...
  const handleCloneAndScan = async () => {
    ...
    if (!window.electronAPI?.cloneScan) {
      await customAlert(
        "Feature Unavailble",
        "Cloning Repositories is only supported inside the desktop app",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        await customAlert(
          "Scan Complete",
          "Clone succeeded but no components or colors were detected. Try Skip to continue without repo context.",
        );
        return;
      }
  ...
  const handleRefinePrompt = async () => {
    ...
    if (!config) {
      await customAlert(
        "Setup Required",
        "Please configure API keys first using the settings menu",
      );
      return;
    }
    setIsRefining(true);
      ...
      (window as any)
        ...
        .then((fontsList: any[]) => {
          const families = Array.from(
            new Set(fontsList.map((f) => f.family)),
          ) as string[];
          families.sort();
    ...
    return (
      <div key={label} className="space-y-1.5">
        ...
        <div className="flex gap-1.5">
          <select
            value={f.fontFamily}
            onChange={(e) => setFontFamily(label, e.target.value)}
              ...
              >
                <div className="h-8 shrink-0 flex items-center gap-3 px-3.5 bg-black/40 border-b border-white/[0.06] select-none">
                  ...
                  <div className="flex-1 max-w-[320px] mx-auto h-[22px] rounded-md bg-white/[0.05] border border-white/[0.04] flex items-center justify-center gap-1.5 overflow-hidden">
                    ...
                    <span className="text-[9px] text-gray-400 tracking-wide truncate">
                      app.kinetic.dev/dashboard
                    </span>
                  </div>
                ...
                <div className="flex-1 min-h-0 flex">
                  <aside
                    className="w-[164px] shrink-0 min-h-0 overflow-hidden flex flex-col justify-between py-3 px-2.5 text-[9px] font-medium tracking-wide border-r border-white/[0.05]"
                  ...
                  >
                    <div className="space-y-3 min-h-0">
                      <div className="flex items-center gap-2 px-1 pb-2 border-b border-white/[0.05]">
                        <img
                          ...
                          style={{
                            filter:
                              "drop-shadow(0 0 8px rgba(139,92,246,0.5)) brightness(1.15)",
                          }}
                        />
                        <span className="flex-1 font-extrabold text-[10.5px] text-white tracking-tight truncate">
                          Kinetic App
                        </span>
                        <CaretDown
                          size={9}
                      ...
                      <div className="space-y-0.5">
                        <span className="block px-1 pb-1 text-[6.5px] font-bold uppercase tracking-[0.18em] text-gray-600">
                          Main
                        </span>
                        <div
                          style={{
                            backgroundColor: `${swatches["Primary"] || "#8b5cf6"}25`,
                            color: swatches["Primary"] || "#8b5cf6",
                          }}
                          className="flex items-center gap-2 px-2 py-1.5 rounded-lg font-semibold"
                        >
                          <House size={12} weight="fill" />
                          <span>Dashboard</span>
                          <span
                            style={{
                              backgroundColor: swatches["Primary"] || "#8b5cf6",
                            }}
                            className="ml-auto w-3.5 h-3 rounded-sm flex items-center justify-center text-[6px] font-bold text-white"
                          >
                            8
                          </span>
                        </div>
                    ...
                    <div className="space-y-2 shrink-0 border-t border-white/[0.05] pt-2.5">
                      ...
                      <div className="flex items-center gap-2 bg-white/[0.04] p-1.5 rounded-lg border border-white/[0.06]">
                        <div
                          style={{
                            background: `linear-gradient(135deg, ${swatches["Primary"] || "#8b5cf6"}, ${swatches["Accent"] || "#f59e0b"})`,
                          }}
                          className="h-6 w-6 rounded-full text-white font-bold flex items-center justify-center text-[8px] shrink-0"
                        >
                          DU
                        </div>
                        <div className="flex flex-col truncate">
                          ...
                          <span className="text-gray-500 text-[7.5px] truncate">
                            Pro Workspace
                          </span>
                        </div>
                      </div>
                  ...
                  >
                    <header className="h-9 shrink-0 px-3 flex items-center justify-between border-b border-white/[0.06] bg-slate-950/70 backdrop-blur-sm">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[8px] font-bold text-white shadow-md shadow-purple-900/30 whitespace-nowrap cursor-pointer"
                        >
                    ...
                    <div className="flex-1 min-h-0 p-3 flex flex-col gap-2.5 justify-between overflow-hidden">
                      <div className="space-y-0.5 shrink-0">
                        <div className="flex items-end justify-between gap-2">
                          <h1
                            style={{
                              ...
                              fontWeight: fonts["Title Font"].bold
                                ? "bold"
                                : "normal",
                          ...
                          >
                            {instructions.trim()
                              ? instructions.slice(0, 42)
                              : "SaaS Product Analytics"}
                          </h1>
                          <span className="shrink-0 mb-0.5 px-1.5 py-px rounded-md bg-emerald-400/10 border border-emerald-400/20 text-[6.5px] font-bold text-emerald-400 uppercase tracking-wider">
                            Live
                          </span>
                        </div>
                        <p
                          style={{
                            ...
                            fontWeight: fonts["Paragraph"].bold
                              ? "bold"
                              : "normal",
                            fontStyle: fonts["Paragraph"].italic
                              ? "italic"
                              : "normal",
                            textDecoration: fonts["Paragraph"].underline
                              ? "underline"
                              : "none",
                        ...
                        >
                          {narration.trim()
                            ? narration.slice(0, 75) + "..."
                            : "Live product walkthrough metrics, ARR growth, and customer engagement."}
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-4 gap-2 shrink-0">
                        {[
                          ...
                          <div
                            key={kpi.label}
                            className="bg-gray-950/80 border border-gray-800 rounded-xl p-2 shadow-sm min-w-0"
                          >
                            <span className="text-[6.5px] uppercase tracking-wider text-gray-400 block font-bold truncate">
                              {kpi.label}
                            </span>
                            <span
                              style={{
                                fontFamily: fonts["Heading"].fontFamily,
                                color: swatches["Secondary"] || "#a78bfa",
                              }}
                              className="text-[13px] font-extrabold block leading-tight"
                            >
                              {kpi.value}
                            </span>
                            <div className="flex items-center justify-between mt-0.5 gap-1">
                              <span
                                className={`text-[6.5px] font-bold whitespace-nowrap ${kpi.tone}`}
                              >
                                {kpi.delta}
                              </span>
                      ...
                      <div className="grid grid-cols-[1.6fr_1fr] gap-2.5 flex-1 min-h-[80px]">
                        <div className="bg-gray-950/90 border border-gray-800 rounded-xl p-2.5 flex flex-col min-h-0">
                          <div className="flex justify-between items-center text-[7.5px] text-gray-400 border-b border-gray-800/60 pb-1.5 shrink-0">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-gray-200 uppercase tracking-wider text-[7px]">
                                Revenue Overview
                              </span>
                              <span className="hidden sm:flex items-center gap-1">
                                <span
                                  ...
                                  style={{
                                    backgroundColor:
                                      swatches["Secondary"] || "#a78bfa",
                                  }}
                                />
                                <span className="text-[6.5px]">MRR</span>
                              </span>
                              <span className="hidden sm:flex items-center gap-1">
                                <span
                                  className="w-1.5 h-1.5 rounded-full"
                                  style={{
                                    backgroundColor:
                                      swatches["Accent"] || "#f59e0b",
                                  }}
                                />
                                <span className="text-[6.5px]">Goal</span>
                              </span>
                            </div>
                            <div className="flex gap-1">
                              {["6M", "1Y", "All"].map((t, i) => (
                                <span
                                  key={t}
                                  className={`px-1.5 py-px rounded ${i === 0 ? "bg-white/10 text-gray-200 font-bold" : "text-gray-500"}`}
                                >
                                  {t}
                                </span>
                              ))}
                          ...
                          <svg
                            ...
                            className="flex-1 min-h-0 w-full mt-1.5"
                          >
                            <defs>
                              <linearGradient
                                id="saasAreaFill"
                              ...
                              >
                                <stop
                                  offset="0%"
                                  stopColor={swatches["Secondary"] || "#a78bfa"}
                                  stopOpacity="0.35"
                                />
                                <stop
                                  offset="100%"
                            ...
                            <path
                              d="M0 78 C24 70 40 82 60 72 C80 62 96 74 116 64 C136 54 152 62 172 50 C192 38 208 44 228 34 C248 26 264 32 284 22 L314 14 L314 100 L0 100 Z"
                              fill="url(#saasAreaFill)"
                            />
                            <path
                              d="M0 88 C24 84 40 90 60 84 C80 78 96 86 116 80 C136 74 152 80 172 72 C192 66 208 70 228 62 C248 56 264 58 284 52 L314 48"
                              fill="none"
                              stroke={swatches["Accent"] || "#f59e0b"}
                              strokeWidth="1.5"
                              strokeDasharray="4 3"
                              opacity="0.55"
                              strokeLinecap="round"
                            />
                            <path
                              d="M0 78 C24 70 40 82 60 72 C80 62 96 74 116 64 C136 54 152 62 172 50 C192 38 208 44 228 34 C248 26 264 32 284 22 L314 14"
                              fill="none"
                              stroke={swatches["Secondary"] || "#a78bfa"}
                              strokeWidth="2"
                              strokeLinecap="round"
                            />
                            <circle
                          ...
                          <div className="flex justify-between px-0.5 shrink-0 pt-1 border-t border-gray-800/60">
                            {[
                              ...
                              <span
                                key={m}
                                className="text-[6px] text-gray-600"
                              >
                                {m}
                              </span>
                            ))}
                        ...
                        <div className="flex flex-col gap-2.5 min-h-0">
                          <div className="bg-gray-950/90 border border-gray-800 rounded-xl p-2 flex flex-col gap-1 flex-1 min-h-0 overflow-hidden">
                            <div className="flex items-center justify-between border-b border-gray-800 pb-1 shrink-0">
                              <span className="font-bold text-gray-200 uppercase tracking-wider text-[6.5px]">
                                Live Audit Stream
                              </span>
                              <DotsThree
                                size={9}
                            ...
                            <div className="space-y-1 overflow-hidden">
                              <div className="flex items-center gap-1.5 bg-gray-900/90 p-1 rounded-lg border border-gray-800">
                                <TrendUp
                                  ...
                                  className="shrink-0"
                                />
                                <span className="text-gray-200 font-medium truncate flex-1 text-[6.5px]">
                                  Enterprise License
                                </span>
                                <span className="text-emerald-400 font-bold text-[6.5px] shrink-0">
                                  +$2,400
                                </span>
                              </div>
                              <div className="flex items-center gap-1.5 bg-gray-900/90 p-1 rounded-lg border border-gray-800">
                                ...
                                <span className="text-purple-400 font-bold text-[6.5px] shrink-0">
                                  200 OK
                                </span>
                              </div>
                              <div className="flex items-center gap-1.5 bg-gray-900/90 p-1 rounded-lg border border-gray-800">
                                ...
                                <span className="text-amber-400 font-bold text-[6.5px] shrink-0">
                                  Joined
                                </span>
                              </div>
                            </div>
                          ...
                          <div className="bg-gray-950/90 border border-gray-800 rounded-xl p-2 shrink-0">
                            <span className="block font-bold text-gray-200 uppercase tracking-wider text-[6.5px] pb-1.5 border-b border-gray-800">
                              Top Regions
                            </span>
                            <div className="mt-1.5 space-y-1">
                              {[
                                ...
                                >
                                  <span className="w-12 shrink-0 text-[6px] text-gray-500 truncate">
                                    {r.name}
                                  </span>
                                  ...
                                  <span className="w-5 text-right text-[6px] font-bold text-gray-400 shrink-0">
                                    {r.pct}%
                                  </span>
                                </div>
                              ))}
          ...
          <div className="w-full space-y-1.5">
            <div className="flex justify-between text-[10px] font-bold text-purple-300">
              <span className="capitalize">
                {pipelineState.status === "repoScan"
                  ? "Waiting for repo scan — check the sidebar"
            ...
            <div className="h-2 w-full overflow-hidden rounded-full bg-gray-950">
              <div
                ...
                style={{
                  width: `${Math.max(3, Math.round(pipelineState.progress * 100))}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-end">
          <div className="w-[450px] h-full bg-gray-950 border-l border-gray-900 p-6 flex flex-col gap-6 overflow-y-auto shadow-2xl">
            ...
            {scannedExports.routes.length > 0 && (
              <div className="space-y-2">
                <span className="text-[10px] font-bold text-gray-500 tracking-wider block">
                  Detected Routes ({scannedExports.routes.length})
                </span>
                <div className="flex flex-wrap gap-1">
                  {scannedExports.routes.map((route) => (
                    <span
                      key={route}
                      className="px-2 py-0.5 rounded bg-gray-900/80 text-gray-300 border border-gray-800 text-[9px] font-mono"
                    >
                      {route}
                    </span>
                  ))}
            ...
            {scannedExports.components.length > 0 && (
              <div className="space-y-2">
                <span className="text-[10px] font-bold text-gray-500 tracking-wider block">
                  Detected Components ({scannedExports.components.length})
                </span>
                <div className="flex flex-wrap gap-1">
                  {scannedExports.components.map((comp) => (
                    <span
                      key={comp}
                      className="px-2 py-0.5 rounded bg-transparent text-purple-300 border border-purple-500/60 text-[9px]"
                    >
                      {comp}
                    </span>
                  ))}
            ...
            {scannedExports.colors.length > 0 && (
              <div className="space-y-2">
                <span className="text-[10px] font-bold text-gray-500 tracking-wider block">
                  Detected Colors ({scannedExports.colors.length})
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {scannedExports.colors.map((color) => (
                    <div
                      key={color}
                      className="flex items-center gap-1.5 bg-gray-900/60 px-2 py-1 rounded border border-gray-800 text-[10px] text-gray-300 font-mono"
                    >
                      <div
            ...
            {scannedExports.fonts.length > 0 && (
              <div className="space-y-2">
                <span className="text-[10px] font-bold text-gray-500 tracking-wider block">
                  Detected Fonts ({scannedExports.fonts.length})
                </span>
                <div className="flex flex-wrap gap-1">
                  {scannedExports.fonts.map((font) => (
                    <span
                      key={font}
                      className="px-2 py-0.5 rounded bg-gray-900/80 text-gray-300 border border-gray-800 text-[9px] font-sans"
                    >
                      {font}
                    </span>
                  ))}
