...
const SaaSGenerator: React.FC<AnimationGeneratorProps> = ({
        ...
        <div className="flex gap-1.5">
          ...
          >
            {availableFonts.map((font) => (
              <option key={font} value={font} className="bg-gray-950 text-white">
                {font}
              </option>
            ))}
          </select>
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-white page-enter">
      ...
      >
        <SidebarHeader breadcrumb="SaaS" onBack={handleBack} />
            ...
            <div className="w-full h-full flex items-center justify-center transition-transform duration-500 ease-out [transform-style:preserve-3d] [backface-visibility:hidden] [will-change:transform] antialiased [transform:rotateX(6deg)_rotateY(-4deg)_scale(0.98)] hover:[transform:rotateX(0deg)_rotateY(0deg)_scale(1)] shadow-[0_30px_70px_rgba(0,0,0,0.8),0_0_40px_rgba(139,92,246,0.15)] rounded-xl border border-purple-500/20 overflow-hidden">
              <div
                ...
                style={{
                  ...
                  fontFamily: (fonts as Record<string, FontSettings>)["Paragraph"].fontFamily,
                }}
              >
                ...
                <div className="flex-1 min-h-0 flex">
                  <aside className="w-[164px] shrink-0 min-h-0 overflow-hidden flex flex-col justify-between py-3 px-2.5 text-[9px] font-medium tracking-wide border-r border-white/[0.05]" style={{ backgroundColor: "#080b12" }}>
                    ...
                    <div className="space-y-2 shrink-0 border-t border-white/[0.05] pt-2.5">
                      ...
                      <div className="flex items-center gap-2 bg-white/[0.04] p-1.5 rounded-lg border border-white/[0.06]">
                        <div style={{ background: `linear-gradient(135deg, ${swatches["Primary"] || "#8b5cf6"}, ${swatches["Accent"] || "#f59e0b"})` }} className="h-6 w-6 rounded-full text-white font-bold flex items-center justify-center text-[8px] shrink-0">DU</div>
                  ...
                  <div className="flex-1 min-w-0 min-h-0 relative flex flex-col" style={{ backgroundImage: backgroundImage ? `url(${backgroundImage})` : undefined, backgroundSize: "cover", backgroundPosition: "center" }}>
                    ...
                    <div className="flex-1 min-h-0 p-3 flex flex-col gap-2.5 justify-between overflow-hidden">
                      <div className="space-y-0.5 shrink-0">
                        <div className="flex items-end justify-between gap-2">
                          <h1
                            style={{
                              fontFamily: (fonts as Record<string, FontSettings>)["Title Font"].fontFamily,
                              color: (fonts as Record<string, FontSettings>)["Title Font"].color,
                              fontWeight: (fonts as Record<string, FontSettings>)["Title Font"].bold ? "bold" : "normal",
                              fontStyle: (fonts as Record<string, FontSettings>)["Title Font"].italic ? "italic" : "normal",
                              textDecoration: (fonts as Record<string, FontSettings>)["Title Font"].underline ? "underline" : "none",
                              fontSize: `${Math.min(17, (fonts as Record<string, FontSettings>)["Title Font"].size / 2.5)}px`,
                            }}
                        ...
                        <p
                          style={{
                            fontFamily: (fonts as Record<string, FontSettings>)["Paragraph"].fontFamily,
                            color: (fonts as Record<string, FontSettings>)["Paragraph"].color,
                            fontWeight: (fonts as Record<string, FontSettings>)["Paragraph"].bold ? "bold" : "normal",
                            fontStyle: (fonts as Record<string, FontSettings>)["Paragraph"].italic ? "italic" : "normal",
                            textDecoration: (fonts as Record<string, FontSettings>)["Paragraph"].underline ? "underline" : "none",
                            fontSize: `${Math.min(9, (fonts as Record<string, FontSettings>)["Paragraph"].size / 1.4)}px`,
                          }}
                      ...
                      <div className="grid grid-cols-4 gap-2 shrink-0">
                        {[
                          ...
                          <div key={kpi.label} className="bg-gray-950/80 border border-gray-800 rounded-xl p-2 shadow-sm min-w-0">
                            ...
                            <span style={{ fontFamily: (fonts as Record<string, FontSettings>)["Heading"].fontFamily, color: swatches["Secondary"] || "#a78bfa" }} className="text-[13px] font-extrabold block leading-tight">{kpi.value}</span>
                          ...
                          <div className="flex justify-between items-center text-[7.5px] text-gray-400 border-b border-gray-800/60 pb-1.5 shrink-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="hidden sm:flex items-center gap-1">
                                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: swatches["Secondary"] || "#a78bfa" }} />
                                <span className="text-[6.5px]">MRR</span>
                              </span>
                              <span className="hidden sm:flex items-center gap-1">
                                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: swatches["Accent"] || "#f59e0b" }} />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-end">
          <div className="w-[450px] h-full bg-gray-950 border-l border-gray-900 p-6 flex flex-col gap-6 overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center border-b border-gray-900 pb-3">
              ...
              <button onClick={() => setShowDetailedPanel(false)} className="h-6 w-6 rounded-md flex items-center justify-center text-gray-500 hover:bg-gray-900 hover:text-white">
                <X size={16} />
              </button>
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {scannedExports.routes.map((route) => (
                    <span key={route} className="px-2 py-0.5 rounded bg-gray-900/80 text-gray-300 border border-gray-800 text-[9px] font-mono">{route}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {scannedExports.components.map((comp) => (
                    <span key={comp} className="px-2 py-0.5 rounded bg-transparent text-purple-300 border border-purple-500/60 text-[9px]">{comp}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {scannedExports.fonts.map((font) => (
                    <span key={font} className="px-2 py-0.5 rounded bg-gray-900/80 text-gray-300 border border-gray-800 text-[9px] font-sans">{font}</span>
                  ))}
                </div>
              </div>
            )}
